...
def main():
    ...
    if "--live" in sys.argv or not any(a.startswith("--") for a in sys.argv[1:]):
        tests.extend([
            ("python -c \"from opencode import opencode; r = opencode('say hi', keep=True); print('R1:', r); r2 = opencode('what did you say?', keep=True); print('R2:', r2); r3 = opencode('translate to russian', keep=False); print('R3:', r3)\"", "Keep mode (multi-turn)"),
            ("python -c \"from opencode import opencode; r = opencode('create file _test_auto.txt with content AutoTest', auto_tools=True, keep=True); print('R:', r)\"", "Auto-tools (file creation)"),
        ])
